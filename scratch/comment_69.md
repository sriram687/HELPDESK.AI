Helo @varshini-nandula! 🙌 (And open to @saurabhhhcodes, @namann5, @harshitanagpal05, @Anand-240, @tamannaa-rath, or any other contributors who want to collaborate in parallel!)
...
1. **PyTest Backend Coverage**: Set up `pytest` and `httpx`, then write tests under `backend/tests/` covering FastAPI route registrations, tenant-isolation enforcement, AI classification mocking, and SLA/duplicate payload validation.
2. **Jest & React Testing Library**: Add frontend unit/component tests inside `Frontend/src/__tests__/` to verify `CreateTicket` pre-fills, `AdminSettings` state/triggers, and theme local storage persistence.
3. **CI/CD Integration**: Integrate automated test checks into a GitHub Actions workflow in `.github/workflows/` to execute tests on every PR targeting `gssoc`.
